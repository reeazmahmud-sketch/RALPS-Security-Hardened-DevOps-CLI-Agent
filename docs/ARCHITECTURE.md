...
3. The agent engine derives safe executable actions from the prompt.
...
6. Autopilot evaluates configured schedules and records recent run history.
...
- MCP support currently provides local JSON-RPC handling for initialization, tool listing, and simple tool calls.
